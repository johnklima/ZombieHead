...
public class HeadRollState : StateNode
{

    public Vector3 velocity;

    //constructor
    public HeadRollState(RootState root)
    {
        rootState = root;
    ...
    public override bool advanceTime(float dt)
    {
        ...
        if (advanceState(dt) == true)
        {
            ...
            rootState.playermotion.roll.enabled = false;
        ...
        {
            //do something
            Debug.Log("IN HEAD ROLL");
            rootState.playermotion.roll.enabled = true;
        }
        else
        {
            //clean up my state
